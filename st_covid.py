...
st.sidebar.checkbox("世卫组织冠状病毒 (COVID-19) 仪表板", True)
...
def get_total_dataframe(dataset):
    total_dataframe = pd.DataFrame({
        ...
        '病例数': (dataset.iloc[0]['New_cases'],
                ...
                dataset.iloc[0]['New_deaths'],
                dataset.iloc[0]['Cumulative_deaths'])})
...
st.markdown("下表为您提供了 %s Covid-19总新增病例、累计病例、新增死亡病例和累计死亡病例的实时分析。"% (select))
